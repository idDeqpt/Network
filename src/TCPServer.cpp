...
std::string net::TCPServer::recv(int socket)
{
	...
	while ((recv_result = ::recv(socket, buf, max_client_buffer_size, WIN(0)NIX(MSG_DONTWAIT))) != 0)
	{
		if (recv_result > -1)
		{
			result += buf;
		...
		else
		{
			if (content_length == -1)
			{
				int field_end = result.find("Content-Length:");
				...
				else
				{
					...
					try 
					{
						int number_end = result.find("\r\n", field_end);
						if (number_end != std::string::npos)
							content_length = stoi(result.substr(field_end, number_end - field_end));
					}catch(...){}
				}
			}
			else if (content_length == 0)
			{
				int end_i = result.find("\r\n\r\n");
				if (end_i != std::string::npos)
					break;
			}
			else
			{
				if (content_length <= (result.size() - result.find("\r\n\r\n") - 4))
					break;
			}
		}
	}
